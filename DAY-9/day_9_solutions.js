...
console.log(
  products.reduce(
    (acc, int) => (Number.isInteger(int.price) ? acc + int.price : acc),
    0
  )
);
...
//   arr.filter((n) => n.toLowerCase().includes(pattern));

// console.log(categorizeCountries(countries, "land"));

// const usedLetter = (arr, l) => {};
//*****7
const findletter = (arr) => {
  return arr.reduce((acc, int) => {
    const letters = int.split("").reduce((acc, int) => {
      if (acc[int]) {
        acc[int] += 1;
      } else {
        acc[int] = 1;
...
console.log(findletter(countries));

// const int = "ahmet, mehmet";
// const letters = int.split("").reduce((acc, int) => {
//   if (acc[int]) {
//     acc[int] += 1;
//   } else {
...
// console.log(country.map((co) => co.name).sort());
// console.log(country.map((c) => c.capital).sort());
// console.log(
//   country
//     .map((c) => c)
//     .sort((a, b) => {
...
// console.log(country.map(co =>co.languages))

const langArray = country.reduce((acc, int, i) => {
  if (!acc) {
    acc = [];
  }
  acc.push(int.languages);
  return acc;
}, []);

const langs = langArray.flat().reduce((accumulator, currentValue) => {
...
country.forEach((con) => {
  con.languages.forEach((language) => {
